<script lang="ts">
	...
	import './methods.css';
	const service = getTradingServices().methods;
	let search = $state(''),
		sort = $state('updated'),
		view = $state<'draft' | 'versions' | 'compare'>('draft');
		...
		<div class="mw-layout">
			<aside class="mw-library" aria-label="Method library">
				...
				<section class="mw-profile" aria-labelledby="profile-title">
					...
					{#if service.profileState === 'loading'}<p>
						...
						>{:else if service.profile?.methodVersionId}<p class="mw-mono">
							{service.profile.methodVersionId}
						</p>
			...
			<section class="mw-main" aria-label="Method authoring and history" aria-busy={busy}>
					...
					{#if view === 'draft'}
						{#if service.content && service.detail.draft}
							<div class="mw-draft-bar">
								<span
									>Draft revision {service.detail.draft.revision} · {service.detail.draft.provenance
										.kind}{#if service.detail.draft.provenance.sourceVersionId}<br /><small
											class="mw-mono"
								...
								<div class="mw-row-actions">
									...
									><button disabled={busy} onclick={() => void service.validate()}>Validate</button
									><button
										class="mw-primary"
										disabled={busy ||
											service.dirty ||
							...
							{#if service.validated}<p
								...
								>
									{service.diagnostics.length
										...
										: 'Definition valid. Evaluation begins in T3.'}
								</p>{/if}
							<MethodEditor
								...
								<div>
									{#if service.selectedVersion}<div class="mw-version-bar">
											...
											<div class="mw-row-actions">
												...
												><button disabled={busy || service.dirty} onclick={() => (dialog = 'clone')}
													>Clone Method…</button
												><button
										...
										<p class="mw-muted">
											Revision: same lineage. Clone: new lineage. Neither changes this version.{#if service.detail.draft}
												A working draft already exists; finish it before creating another revision.{/if}
										</p>
										<MethodSnapshot
					...
					{:else if service.detail.versions.length < 2}<div class="mw-empty-panel">
						...
						{#if service.comparison}<section aria-label="Semantic version comparison">
								...
								{#if !service.comparison.changes.length}<p>
										Canonical content is identical. Storage identity and provenance remain distinct.
									</p>{/if}
								<ul class="mw-changes">
									{#each service.comparison.changes as change, i (i)}<li>
											...
											<div>
												...
												<p><strong>After</strong><span>{readableComparison(change.after)}</span></p>
											</div>
										</li>{/each}
								</ul>
								<details>
				...
				{:else}<div class="mw-empty-panel">
						...
						<p class="mw-muted">Definition available · Evaluation begins in T3</p>
					</div>{/if}
			</section>
		</div>
