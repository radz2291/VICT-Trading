...
| 9   | Saved workspace layouts                               | **SUPPORTED THROUGH VICT COMPOSITION**                                                     | Persist as typed resources over `@victframework/appdata-sqlite` (versioned migrations, `appdata_*` namespace, restart recovery — Stage 05 §10, §16.3). Layout _content_ is product data; VICT provides the storage, contracts, and query surface.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
- **`packages/trading-domain` depends on nothing** — no product package and
  ...
  _patterns_ (declarations, contracts, deterministic identity) may be
...
- **GAP-CANDIDATE-1: subscription data binding for application views.** VICT
  ...
  neutral way for a _view_ to receive pushed updates. Whether this is a
...
- **GAP-CANDIDATE-2 (BLOCKING for T1): declared navigation-group ordering.**
  ...
  express the intended order _of the groups_: the canonical renderer sorts
  ...
  - **CLOSED — `GAP-CANDIDATE-2 CLOSED BY VERIFIED PUBLIC VICT 0.1.1`**
    (recorded during T1 implementation, 2026-09): public VICT `0.1.1`
    (release set `vict-release-set@1/0.1.1`, content ID
    `v1_e31e8dd60d05e1d6feb08b5ed0874cceae561bdf10e08d8b93e07840de8d9cdf`,
    verified by recomputation from the registry manifest) ships a canonical
    renderer that emits navigation groups in **first-occurrence route
    order**: the group sequence follows the order in which routes belonging
    to each group first appear in the Application Definition, with the
    group-less leading entries (Desk, Markets) rendered first — exactly the
    neutral, definition-expressible ordering this gap demanded, with no
    alphabetical re-sorting applied when routes are declared in order.
    Verified in-repo during T1 by unit tests on the compiled
    plan/navigation order and by real-browser assertions on the rendered
    group sequence (`docs/evidence/t1/`; T1 report §navigation order). The
    historical finding above is preserved unchanged.
...
1. **Props-bounded islands and data flow.** The closed-over-services pattern
   ...
   scaffolder host. _Risk: medium; mitigated by T1 acceptance criteria._
2. **Live Watch update path.** Polling may be adequate; if not, GAP-CANDIDATE-1
   becomes a real framework proposal. _Decision deferred to T6 with evidence._
3. **Canvas rendering vs. renderer conformance.** A canvas-based island must
   ...
   the island's own safe states. _Open design detail._
4. **Workspace layout schema.** What exactly persists (panel identity, sizes,
   positions, bindings) needs a small, versioned design at T1. _Open, blocks
   nothing — T1 defines it._
5. **Method identity hashing.** Reuse of VICT's deterministic canonical-hash
   ...
   (`trading.method-version@1`) is defined at T2. _Open, non-blocking._
6. **Node/browser boundary.** `@victframework/application` is browser-safe;
   ...
   must not import Node-only modules into client bundles. _Standing rule._
7. **Pre-1.0 VICT.** `0.x` semver permits breaking minor bumps; exact pins
   ...
   versioned events with re-verification. _Accepted posture._
8. **Upstream dependency: navigation-group ordering (GAP-CANDIDATE-2).** T1
   ...
   that withdraws the central-loop-order claim. _Blocking; owned by the T1
   entry gate (Roadmap)._
